...
import { ShoppingBag, LogOut, ChefHat, LayoutDashboard, ClipboardList, Globe } from "lucide-react";
          ...
          {user && user.role === "company" && (
            <>
              ...
              <Link to="/orders" className="hidden sm:inline-flex text-sm font-medium text-[#5C5855] hover:text-[#C05A46] px-3 py-2 transition-colors" data-testid="nav-orders">
                <ClipboardList size={16} className="mr-1.5" /> {t("nav_orders")}
              </Link>
              {onCartClick && (
                <Button
                  onClick={onCartClick}
                ...
                >
                  <ShoppingBag size={16} className="mr-2" /> {t("nav_cart")}
                  {cart.count > 0 && (
