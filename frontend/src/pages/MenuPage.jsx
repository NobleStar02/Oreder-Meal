...
import { Plus, Minus, ShoppingBag, X, Utensils, Trash2 } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-[#F9F6F0]">
      ...
      <section className="max-w-7xl mx-auto px-5 md:px-8 pb-24">
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            {items.map((it, idx) => (
              <MenuCard key={it.id} item={it} idx={idx} />
            ))}
          </div>
        )}
      </section>
        ...
        <SheetContent className="w-full sm:max-w-md bg-[#F9F6F0] border-l border-[#E5DFD3] flex flex-col p-0" data-testid="cart-sheet">
              ...
              <ul className="space-y-3">
                {cart.items.map((i) => (
                  <li key={i.id} className="flex items-center gap-3 bg-white rounded-xl border border-[#E5DFD3] p-3" data-testid={`cart-item-${i.id}`}>
                    {i.image_path ? (
                      <img src={fileUrl(i.image_path)} alt="" className="w-14 h-14 rounded-lg object-cover" />
                    ) : (
                      <div className="w-14 h-14 rounded-lg bg-[#F2EBE3] grid place-items-center"><Utensils size={20} className="text-[#C05A46]" /></div>
                    )}
                    <div className="flex-1 min-w-0">
                      <div className="font-semibold text-[#2C2A29] truncate">{i.name}</div>
                      <div className="text-sm text-[#8A8580]">{formatTRY(i.price)} × {i.quantity}</div>
                    </div>
          ...
          <div className="border-t border-[#E5DFD3] px-6 py-5 bg-white">
            <div className="flex items-center justify-between mb-4">
              <span className="text-sm uppercase tracking-[0.2em] font-bold text-[#8A8580]">Toplam</span>
              <span className="font-heading text-2xl font-bold" data-testid="cart-total">{formatTRY(cart.total)}</span>
            </div>
            <Button
              onClick={placeOrder}
      ...
      {cart.count > 0 && !cartOpen && (
        ...
        >
          <ShoppingBag size={18} /> {cart.count} ürün · {formatTRY(cart.total)}
        </button>
      )}
    </div>
  );
    ...
    >
      ...
      <div className="p-5 flex-1 flex flex-col">
        ...
        <div className="mt-auto pt-5 flex items-center justify-between gap-3">
          <div className="font-heading text-2xl font-bold text-[#C05A46]">{formatTRY(item.price)}</div>
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              className="bg-[#2C2A29] hover:bg-[#1a1918] text-white rounded-full px-4 h-9"
